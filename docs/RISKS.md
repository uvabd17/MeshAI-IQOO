...
| K19 | Credited replacement plan over-estimates what a stop frees on the laptop (mmap'd weights partly live in page cache) | Med | Low | run-guards | Plan estimate credited (D024); if llama-server then fails to load the run ends in `error` and the user re-runs; Phase 2: measure MemAvailable before/after a stop and calibrate | T012 |
