...
2026-09-24 · The phone host fetches the model from `/api/models/file/…`, which is loopback-only by default. **Decision:** `POST /api/run` answers 422 for a remote host unless `--lan` is set. Before the current run is stopped: cheap checks (model exists, `0 < n_ctx ≤ n_ctx_train`, host known/online, remote host needs `--lan`) and a *dry-run plan* against the current memory view; only a memory shortfall (DoesNotFit / no eligible device) may proceed, because it can resolve once the old run releases its memory. Then stop → re-plan against the freed memory (M4) → start. If the re-plan still fails the 422 body and the run log say "previous run stopped; new plan failed: …" (round 6). Status: accepted.
