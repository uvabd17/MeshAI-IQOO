...
2026-09-24 · The phone host fetches the model from `/api/models/file/…`, which is loopback-only by default. **Decision:** `POST /api/run` answers 422 for a remote host unless `--lan` is set. Only *cheap* checks (model exists, host known/online, remote host needs `--lan`) run before the current run is stopped; the plan itself is made after the stop so the planner sees the freed memory (M4, "don't split if it fits") — round 5 caught the regression where planning before the stop saw ~1 GB less on the laptop. Status: accepted.
