...
## D024 — A phone can be the host only when meshd runs with `--lan --api-token`; a replacement run is planned ONCE, before the stop, crediting only memory the live run demonstrably holds
2026-09-24 · The phone host fetches the model from `/api/models/file/…`, which is loopback-only by default, so `POST /api/run` answers 422 for a remote host unless `--lan` is set. **Replacement runs (rounds 4–9):** planning after the stop saw stale memory (phones report every 2 s; round 8); planning before the stop saw memory the old run still held (round 5); crediting the plan's *estimate* credited memory not yet held (round 9: a second Run during `loading` was credited 5.32 GB on a laptop whose MemAvailable had not moved — 12.49 GB before, 12.48 GB during load; measured by the round-9 audit on this laptop, Qwen3-8B Q4_K_M, ctx 2048). **Decision:** `api_run` does cheap checks (model exists, `0 < n_ctx ≤ n_ctx_train`, host known/online, remote host needs `--lan`), then makes exactly one plan against `credited_caps()`. A device is credited only when the run is `ready`, the device is online with *measured* memory (no override) sampled after `ready_ms`, and the credit is `min(placement bytes, avail at start − avail now)` — the drop actually observed. Anything else (loading run, stale sample, override, offline, rejected placement) earns nothing. If the plan fails the request is refused and the live run is untouched; if it succeeds: stop → start that plan, no re-plan. `/api/plan` previews with the same capacities. Measured on this laptop by the round-9 audit: the 8B run released 3.56 GB of a 5.32 GB placement at stop (11.03 → 14.59 GB MemAvailable), so the old estimate would have over-credited ~1.8 GB; the observed-drop rule cannot. With mmap an over-committed load does *not* fail cleanly on Linux — it pages or the OOM killer intervenes — which is why the credit is evidence-based (K19). Status: accepted; unit-tested (`credit_tests`, incl. the planner end to end); executed by `scripts/qa/run-guards.sh` (no credit while loading; credit ≤ placement after ready).
