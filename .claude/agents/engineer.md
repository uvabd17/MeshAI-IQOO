...
Before writing code: read `CLAUDE.md`, the task's entry in `state/tasks.json`, and the relevant section of `docs/ARCHITECTURE-v3-native-mesh.md`. If you are touching `android/`, the `android-native` skill loads automatically — follow its API references rather than memory.
...
- Every measured number goes to `docs/BENCHMARKS.md` with model, quant, context, backend, link, device.
